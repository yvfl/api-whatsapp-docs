...
<!-- Scraped: 2026-01-24T01:10:47.041Z -->
...
-   Sua mensagem é enviada a um usuário do WhatsApp.-   Sua mensagem é entregue ao dispositivo de um usuário do WhatsApp.-   Sua mensagem é exibida (ou seja, "lida") no cliente do WhatsApp no dispositivo de um usuário do WhatsApp.-   Não é possível enviar sua mensagem a um usuário do WhatsApp.-   Não é possível entregar a mensagem ao dispositivo de um usuário do WhatsApp.-   Sua mensagem é enviada a um usuário do WhatsApp em uma conversa em grupo.-   A mensagem de voz é reproduzida pelo dispositivo do usuário do WhatsApp.

Os gatilhos acima também se aplicam a usuários do WhatsApp que fazem parte de uma conversa em grupo.

Um status é considerado lido somente se tiver sido entregue. Em alguns casos (por exemplo, quando o usuário recebe uma mensagem enquanto está na tela da conversa), a mensagem é entregue e lida ao mesmo tempo. Nesses casos, o webhook "entregue" não é enviado, porque fica implícito que a mensagem foi entregue depois de ter sido lida. Esse comportamento é resultado de uma otimização interna.
...
[Categoria da conversa](/documentation/business-messaging/whatsapp/pricing#conversation-categories). Os valores podem ser os seguintes:
...
`marketing_lite`: indica uma conversa da [API de Mensagens de Marketing para WhatsApp](/documentation/business-messaging/whatsapp/marketing-messages/overview).
...
Detalhes do [código de erro](/documentation/business-messaging/whatsapp/support/error-codes).
...
Título do [código de erro](/documentation/business-messaging/whatsapp/support/error-codes). Esse valor é igual ao valor da propriedade `message`.
...
`marketing_lite`: indica a aplicação de preços da [API de Mensagens de Marketing Lite](/docs/whatsapp/marketing-messages-lite-api).
...
-   Equivalente na interface do usuário do WhatsApp: dois selos de verificação.
...
-   Equivalente na interface do usuário do WhatsApp: um selo de verificação.
...
O valor corresponde ao número de telefone do usuário do WhatsApp, quando a mensagem é enviada diretamente a ele ou à [identificação do grupo](/documentation/business-messaging/whatsapp/groups) quando ela é enviada a uma identificação do grupo. Nesse último caso, o número de telefone do usuário do WhatsApp será atribuído à propriedade `recipient_participant_id`.
...
Este exemplo de webhook v24.0 descreve uma mensagem de marketing exibida no WhatsApp do cliente (ou seja, "lida"). Nesse caso, o objeto `conversation` foi omitido porque se trata de um webhook v24.0. Já o objeto `pricing` foi omitido porque estava sendo exibido em um webhook de mensagens de status entregues associado (o objeto pode aparecer apenas em um ou no outro).
